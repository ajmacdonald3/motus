...
ensureDBTables2 = function(src, projRecv, deviceID, quiet = FALSE) {
  ...
  for(t in c("activity", "activityAll", "antDeps", "batches", "batchRuns", "clarified", "filter", "gps", "gpsAll", "hits", 
             "nodeData", "nodeDeps", "projAmbig", "projs", "pulseCounts",
             "recvDeps", "runs", "runsFilters", "species",
             "tagAmbig", "tagDeps", "tagProps", "tags")) {
    message(t)
    if(!t %in% tables) lapply(makeTable(t), DBI::dbExecute, conn = con)
  }
